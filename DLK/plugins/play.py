...
Contains handlers for /play, /skip, /queue and /stop operations.
This file imports helpers from bot.py so handlers remain small and
...
# import the core objects and helper functions from bot.py
from bot import (
    bot,
    assistant,
    extract_audio_url,
...
async def cmd_play(_, message: Message):
    ...
    Delegates the heavy lifting to helpers available in bot.py.
    """
    ...
    # ensure assistant present (bot.py has redundant checks; reuse here by trying to play)
    entry = None
    info_msg = None
